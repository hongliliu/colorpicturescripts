...
import os
os.chdir('/Volumes/disk4/orion/GeminiSouth/')
...
if not 'fe' in locals():
    ...
    if os.path.exists('Fe_NS_mosaic_bgm_unsharp.fits'):
        ...
        fef[0].data = fenormed
        fef.writeto("Fe_NS_mosaic_bgm_normed.fits",clobber=True)
    # too big fesmooth = astropy.smooth(fe,100,ignore_nan=True)
    #b2 = pyfits.open('GEMS_B2_OrionBNKL_mosaic_bgmatch.fits')
    h2f = pyfits.open('H2_NS_mosaic_bgm.fits')
    h2 = h2f[0].data
    if os.path.exists('H2_NS_mosaic_bgm_unsharp.fits'):
        h2unsharp = pyfits.getdata('H2_NS_mosaic_bgm_unsharp.fits')
        h2normed = pyfits.getdata('H2_NS_mosaic_bgm_normed.fits')
    else:
        #kernel = astropy.nddata.convolution.make_kernel.make_kernel([151,151],kernelwidth=50)
        #h2smooth = astropy.nddata.convolve(h2,kernel)
        h2smooth = agpy.smooth(h2[::4,::4],250, fft_pad=False, interpolate_nan=True, psf_pad=False,
                ignore_edge_zeros=True, normalize_kernel=True, use_numpy_fft=True, nthreads=1,
                use_rfft=True, complextype=np.float32, silent=False, boundary='wrap')
        h2unsharp = h2
        for ii in xrange(4):
            for jj in xrange(4):
                h2unsharp[ii::4,jj::4] -= h2smooth
        h2f[0].data = h2unsharp
        h2f.writeto("h2_NS_mosaic_bgm_unsharp.fits",clobber=True)
        h2normed = h2
        for ii in xrange(4):
            for jj in xrange(4):
                h2normed[ii::4,jj::4] /= h2smooth
        h2f[0].data = h2normed
        h2f.writeto("h2_NS_mosaic_bgm_normed.fits",clobber=True)
    # too big h2smooth = AG_fft_tools.smooth(h2,100,ignore_nan=True)
    ks = pyfits.getdata('Ks_NS_mosaic_bgm.fits')
...
for logii in xrange(0,2):
    ...
    kbackground_contrast.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_blackbg_contrast_small.png' % (logii))
    kbackground_bright = ImageEnhance.Brightness(kbackground_contrast).enhance(1.5)
    kbackground_bright.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_blackbg_contrast_bright_small.png' % (logii))
...
for logii in xrange(0,2):
    ...
    print "Saving GEMS mosaic ",time.time()-t0
    im.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_large.png' % logii)
    print "Fin large image %i.  t=%0.1f s" % (logii,time.time()-t0)

    kbackground = PIL.Image.new("RGB", im.size, (0, 0, 0))
    kbackground.paste(im, mask=im.split()[3])
    print "Saving GEMS mosaic with black bg ",time.time()-t0
    #kbackground.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_blackbg_large.png' % (logii))
    kbackground_contrast = ImageEnhance.Contrast(kbackground).enhance(1.5)
    kbackground_contrast.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_blackbg_contrast_large.png' % (logii))
    kbackground_bright = ImageEnhance.Brightness(kbackground_contrast).enhance(1.5)
    kbackground_bright.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_blackbg_contrast_bright_large.png' % (logii))
...
for hue in linspace(0,330,12):
    ...
    print "saving gems mosaic ",time.time()-t0
    im.save(prefix+'orionbnkl_gems_mosaic_logexp%i_hue%i_small.png' % (logii,hue))
    print "fin small image %i.  t=%0.1f s" % (logii,time.time()-t0)

    print "saving gems mosaic with black bg ",time.time()-t0
    #kbackground.save(prefix+'orionbnkl_gems_mosaic_logexp%ii_blackbg_small.png' % (logii))
    kbackground_contrast = ImageEnhance.Contrast(im).enhance(1.5)
    kbackground_contrast.save(prefix+'orionbnkl_gems_mosaic_logexp%i_hue%i_blackbg_contrast_small.png' % (logii,hue))
    kbackground_bright = ImageEnhance.Brightness(kbackground_contrast).enhance(1.5)
    kbackground_bright.save(prefix+'orionbnkl_gems_mosaic_logexp%i_hue%i_blackbg_contrast_bright_small.png' % (logii,hue))
...
for hue in linspace(0,300,6):
    ...
    im = PIL.Image.fromarray(rgb_float_pil)
    print "Saving GEMS mosaic ",time.time()-t0
    im.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%i_hue%i_rgswap_small.png' % (logii,hue))
    print "Fin rgswap_small image %i.  t=%0.1f s" % (logii,time.time()-t0)

    print "Saving GEMS mosaic with black bg ",time.time()-t0
    #kbackground.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%ii_blackbg_rgswap_small.png' % (logii))
    kbackground_contrast = ImageEnhance.Contrast(im).enhance(1.5)
    kbackground_contrast.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%i_hue%i_blackbg_contrast_rgswap_small.png' % (logii,hue))
    kbackground_bright = ImageEnhance.Brightness(kbackground_contrast).enhance(1.5)
    kbackground_bright.save(prefix+'OrionBNKL_GEMS_mosaic_logexp%i_hue%i_blackbg_contrast_bright_rgswap_small.png' % (logii,hue))
...
for h2x,fex,ksx,txt in ((h2normed,fenormed,ks,"normed_"),(h2,fe,ks,""),(h2unsharp,feunsharp,ks,"unsharp_")):
    for downsample,size in ((4,'small'),(1,'large')):
        h2s = np.zeros([myshape[0]/downsample,myshape[1]/downsample,3],dtype='float')
        if txt == "normed_":
            minv = -0.5
            maxv = 2
            logii = None
        ...
        if txt == "normed_":
            ...
            maxv = femax
            logii = 1
        fes = np.zeros([myshape[0]/downsample,myshape[1]/downsample,3],dtype='float')
        ...
        kss = np.zeros([myshape[0]/downsample,myshape[1]/downsample,3],dtype='float')
        ...
        im.save(prefix+'OrionBNKL_GEMS_mosaic_redblueorange_%s%s.png' % (txt,size))
        im = ImageEnhance.Contrast(im).enhance(1.5)
        im.save(prefix+'OrionBNKL_GEMS_mosaic_redblueorange_%s%s_contrast.png' % (txt,size))
        im = ImageEnhance.Brightness(im).enhance(1.5)
        im.save(prefix+'OrionBNKL_GEMS_mosaic_redblueorange_%s%s_contrast_bright.png' % (txt,size))
...
im.save(prefix+'OrionBNKL_GEMS_mosaic_eq_large.png')
imsm = PIL.Image.fromarray((rgb_eq_sm*255).astype('uint8'))
im.save(prefix+'OrionBNKL_GEMS_mosaic_eq_small.png')
...
kbackground_contrast.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_blackbg_contrast.png')
kbackground_bright = ImageEnhance.Brightness(kbackground_contrast).enhance(1.5)
kbackground_bright.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_blackbg_contrast_bright.png')
...
print "Saving GEMS mosaic ",time.time()-t0
im2.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_rotated.png')
kbackground2 = PIL.Image.new("RGB", im2.size, (0, 0, 0))
kbackground2.paste(im2, mask=im2.split()[3])
print "Saving GEMS mosaic with black bg ",time.time()-t0
kbackground2.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_rotated_blackbg_small.png',quality=10)
print "Saving Bolocam + GEMS mosaic with black bg (PIL) ",time.time()-t0
kbackground2_contrast = ImageOps.autocontrast(kbackground2)
kbackground2_contrast.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_rotated_blackbg_contrast.png')
kbackground2_bright = ImageEnhance.Brightness(kbackground2_contrast).enhance(1.5)
kbackground2_bright.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_rotated_blackbg_contrast_bright.png')
kbackground2_small_bright = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small_bright.save(prefix+'OrionBNKL_4096sq_GEMS_mosaic_rotated_blackbg_bright_small.png',quality=10)
#kbackground2_bright.paste(boloim, mask=boloim.split()[3])
kbackground2_bright.save(prefix+'OrionBNKL_4096sq_GEMS_bolo_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'OrionBNKL_4096sq_GEMS_bolo_mosaic_rotated_blackbg_small.png',quality=10)
