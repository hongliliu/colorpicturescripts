...
import os
os.chdir('/Volumes/disk4/l111/')

if not 'b1' in locals():
    import pyfits
    b1 = pyfits.open('WISE_B1_L111_mosaic_bgmatch.fits')
    #b2 = pyfits.open('WISE_B2_L111_mosaic_bgmatch.fits')
    b3 = pyfits.open('WISE_B3_L111_mosaic_bgmatch.fits')
    b4 = pyfits.open('WISE_B4_L111_mosaic_bgmatch.fits')
    v2 = pyfits.open('L111_MOSAIC_nocrop_proj.fits')
    v2noise = pyfits.open('L111_MOSAIC_NOISE_proj.fits')
...
# or do this: 
b1min = 6 ; b1max=185; b1scale=3.0
b2min = 9 ; b2max=450; b2scale=3.0
b3min = 355; b3max=5600; b3scale=4.0 #1.25
b4min = 97; b4max=700; b4scale=4.0 #0.86
...
savefig(prefix+"L111_WISE_mosaic.png",dpi=300)
imshow(v2img)
print "Saving RGB +bolo mosaic ",time.time()-t0
savefig(prefix+"L111_WISE_mosaic_bolocam.png",dpi=300)
draw()
...
savefig(prefix+"L111_WISE_mosaic_bolocam_try2.png",dpi=300)
...
savefig(prefix+'L111_bolocam_orange.png',dpi=300)
colorbar()
draw()
...
im1.save(prefix+'L111_4096sq_WISE_mosaic.png')
#rgb_pil = ((1-rgb[:,:,:3])*(2**8))
#rgb_pil -= (256*rgb[:,:,3])[:,:,newaxis]
#rgb_pil[rgb_pil>255] = 255
#rgb_pil = rgb_pil.astype('uint8')
#im1 = PIL.Image.fromarray(rgb_pil)
#im1.save(prefix+'L111_4096sq_WISE_mosaic_try2.png')
...
wbackground.save(prefix+'L111_4096sq_WISE_mosaic_whitebg.png')
...
print "Saving WISE mosaic ",time.time()-t0
im2.save(prefix+'L111_4096sq_WISE_mosaic_rotated.png')
kbackground2 = PIL.Image.new("RGB", im2.size, (0, 0, 0))
kbackground2.paste(im2, mask=im2.split()[3])
print "Saving WISE mosaic with black bg ",time.time()-t0
kbackground2.save(prefix+'L111_4096sq_WISE_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'L111_4096sq_WISE_mosaic_rotated_blackbg_small.png',quality=10)
print "Saving Bolocam + WISE mosaic with black bg (PIL) ",time.time()-t0
kbackground2_contrast = ImageOps.autocontrast(kbackground2)
kbackground2_contrast.save(prefix+'L111_4096sq_WISE_mosaic_rotated_blackbg_contrast.png')
kbackground2_bright = ImageEnhance.Brightness(kbackground2_contrast).enhance(1.5)
kbackground2_bright.save(prefix+'L111_4096sq_WISE_mosaic_rotated_blackbg_contrast_bright.png')
kbackground2_small_bright = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small_bright.save(prefix+'L111_4096sq_WISE_mosaic_rotated_blackbg_bright_small.png',quality=10)
kbackground2_bright.paste(boloim, mask=boloim.split()[3])
kbackground2_bright.save(prefix+'L111_4096sq_WISE_bolo_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'L111_4096sq_WISE_bolo_mosaic_rotated_blackbg_small.png',quality=10)
