...
import os
os.chdir('/Volumes/disk4/orion/GeminiSoutheast/')
...
if not 'fe' in locals():
    ...
    h2f = pyfits.open('h2_montage_georgs_southeast.fits')
    h2 = h2f[0].data
    if os.path.exists('h2_montage_georgs_southeast_unsharp.fits'):
        h2unsharp = pyfits.getdata('h2_montage_georgs_southeast_unsharp.fits')
        h2normed = pyfits.getdata('h2_montage_georgs_southeast_normed.fits')
    else:
        #kernel = astropy.nddata.convolution.make_kernel.make_kernel([151,151],kernelwidth=50)
        ...
        h2f.writeto("h2_montage_georgs_southeast_unsharp.fits",clobber=True)
        h2normed = h2
        for ii in xrange(4):
            for jj in xrange(4):
                h2normed[ii::4,jj::4] /= h2smooth
        h2f[0].data = h2normed
        h2f.writeto("h2_montage_georgs_southeast_normed.fits",clobber=True)
    # too big h2smooth = AG_fft_tools.smooth(h2,100,ignore_nan=True)
    ks = pyfits.getdata('ks_montage_georgs_southeast.fits')
